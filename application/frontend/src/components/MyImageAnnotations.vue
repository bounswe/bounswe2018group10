      ...
      <b-row class="mb-4">
        <b-col>
          <b-card no-body class="border-0 shadow">
            <b-list-group flush>
              <b-list-group-item :key="anno.id" v-for="anno in imageAnnotations">
                ...
                <router-link :to="removeBaseUrl(anno.target.scope)">Go to annotated page</router-link>
              </b-list-group-item>
            </b-list-group>
          </b-card>
        </b-col>
...
export default {
  ...
  methods: {
    fetchData() {
      this.$axios
        .get(`/annotation/imageannotation/?user__id=${this.$root.$data.user_id}`)
        .then(response => {
